...
| R4 | **Open:** lifecycle/deletion critical path; the admitted disjoint analytical-value lane is complete, while later flow/feedback cards remain outside the active horizon | Completed: DL-LIFE-01 + DL-EVQ-03 + LIFE-02 slice A + B1a. Active now: B1a late-review repair, then DL-LIFE-02 B1b–B4. Later: DL-SPINE-05, DL-BRIDGE-02, DL-FLOW-01/02/04, DL-OBSV-01/03, DL-CI-01 (after the complete deletion planner) |
...
reassessment admits them. LIFE-02's registered-SQLite slice A and inert B1a contract are merged,
but the launcher (`09_IMPLEMENTATION_LAUNCHER.md`) keeps the card open through the bounded B1a
late-review repair, B1b-i/ii/iii identity and migration, B2 retention/resolver, B3 complete SQL
deletion, and B4 app-owned artifacts as bound by
