...
| R4 | **Open:** lifecycle/deletion critical path; the admitted disjoint analytical-value lane is complete, while later flow/feedback cards remain outside the active horizon | Completed: DL-LIFE-01 + DL-EVQ-03 + LIFE-02 through B2b-ii-j (the inert B2b continuity chain was deleted by the 2026-08-05 §7 simplification). Active now: DL-LIFE-02 (current sub-slice tracked in `CURRENT_STATE.md`). Later: DL-SPINE-05, DL-BRIDGE-02, DL-FLOW-01/02/04, DL-OBSV-01/03, DL-CI-01 (after the complete deletion planner) |
...
reassessment admits them. The card stays open through the remaining LIFE-02 work as bound by
`10_LIFE_02B_DECISION.md` (§7 defines the current executable stopping point); the current
sub-slice is tracked in `CURRENT_STATE.md`, which is the single resume artifact.
...
128 cards. Generated by `taskdeck/tools/generate.mjs` from `taskdeck/tools/cards.mjs` — edit the
...
| DL-LIFE-01 | Capability lifecycle state machine + approval-never-activates invariant | lifecycle | contract | DONE |  | none | M1 | high/M |
| DL-LIFE-02 | Deletion enumeration from schema registry + cascade proof | lifecycle | implementation | READY | A | DL-LIFE-01 | M1 | high/M |
...
| DL-Q-PROSE | Constitution record: PR/issue prose eligibility (non-activation) | open-questions | process | DONE |  | none | M7 | high/S |
| DL-Q-INDEX | Constitution record: durable retrieval index eligibility (non-activation) | open-questions | process | DONE |  | none | M7 | medium/S |
| DL-Q-LOCALMODEL | Constitution record: pinned offline local-model option (D12=NO) | open-questions | process | DONE |  | none | M8 | medium/S |
| DL-Q-CONSENT | Constitution record: consented validation dataset eligibility (per candidate) | open-questions | process | DONE |  | none | M6 | high/S |
...
| DL-EVQ-03 | Claim stability across re-collections (frontier C-03; stability key) | evidence-quality | implementation | DONE |  | DL-SPINE-01, DL-SPINE-02 | M6 | medium/M |
...
| DL-Q-AGENTCFG | Constitution record: agent_config presence-only role (D5=PRES) | open-questions | process | DONE |  | none | M6 | medium/S |
| DL-Q-AGENTCFG-TIMING | Owner gate: agent_config portfolio adoption-timing suppression (frontier A14) | open-questions | process | QUESTION |  | none | M6 | medium/S |
