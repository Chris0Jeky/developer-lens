...
# Developer Lens Continuation (Claude runtime)
...
## Select, bound, and route the slice
...
3. Keep orchestration, authority interpretation, and architecture in the coordinating session.
   Delegate bounded implementation to `dl-implementer`, fresh-context adversarial review to
   `dl-reviewer`, and mechanical sweeps to `dl-mechanic` (definitions in `.claude/agents/`).
4. Use one writer per checkout; parallel write lanes require separate coordinator-owned worktrees
   and non-overlapping paths. Subagents can move HEAD — pin git state in every delegation prompt
   and re-verify branch/HEAD after each completion.
5. Use invented fixtures first. An approved real migration or sensitive-source task must still
...
6. Treat missing permission or censored history as explicit coverage, never zero activity.
7. For external-model work, stay inside the approved OpenAI/Luna data-charter boundary and keep
   `cap.external.model` `never_authorized` until a bounded default-off implementation and
   exact-head proving gate pass. Approval never authorizes an unreviewed payload.
...
1. Run the narrow test from `CLAUDE.md`, then `npm run check` for a code/config milestone.
2. Run `npm run verify:context` for docs, authority, prompt, or skill changes.
3. Review the exact diff against the charter and matrix; use one fresh-context adversarial pass
   (`dl-reviewer`) for non-trivial privacy or logic work.
