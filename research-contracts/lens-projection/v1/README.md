...
- `repositories`: ≤ 12, ordered by `attentionShare` descending, labels unique. `label` ≤ 40; a
  `private-alias` or `masked-alias` label matches `^Project [A-Z][a-z]+(?: [1-9][0-9]{0,2})?$`.
  `primaryLanguage` (≤ 32) is omitted when no language was detected. `attentionShare` is `0..1`;
  `activeWeeks`, `mergedPullRequests`, `reviews` are integers; `momentum` is `-1..1` (the
  payload's `-100..100` ÷ 100).
...
- Shares: `repositories[].attentionShare` and `themes[].share` are apportioned in thousandths by
  ...
  so each set sums to exactly 1 (or all 0) and never exceeds the 1.0001 bound. A projected share
  can therefore differ from the payload's independently rounded share by at most 0.001.
...
- `projectionHash`: `sha256:56dd5c3e6d93c0879bc1f058fbbaa058aa14df60596cb6afd9f1e9f0cc90ba7a`
- `inputHash`: `sha256:9beca7c86edce77871e4384da14253324862e8abf36cf41d3bbee71afa8be039`
- complete fixture file SHA-256 (LF line endings, trailing newline):
  `sha256:e1cfb559edc5c92bb892ce953b57a9e9218697c5e83585d218cffa2524b2fd11`
