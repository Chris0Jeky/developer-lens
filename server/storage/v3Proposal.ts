...
export const STORAGE_V3_DISPOSITIONS = [
  ...
  disposition({ tableName: 'v2_coverage_record', family: 'bridge', action: 'delete', preserve: [], rewrite: [], delete: ['all rows: the v2 reader refuses v3 stores by application_id/user_version so a preserved copy is unreadable by construction, and its verbatim scope_alias would carry C2 into a store whose coverage truth is coverage_ledger (B3 decision)'], refuse: ['rows without exactly one validated C0 synthetic-only provenance record, or under activation_card/unverifiable provenance — source readability is still proven before deletion'] }),
  ...
  disposition({ tableName: 'claim', family: 'claim', action: 'rewrite', preserve: ['claim layer, statement, analytical window and stability facts as C1'], rewrite: ['cl_ ID under claim-id.v3 and canonical scope', 'created_at as expiring C2 operational provenance, cleared at the inclusive 13-month boundary while the C1 claim row remains'], delete: ['the exact created_at value at C2 expiry, never the C1 claim row for that reason', 'claims intentionally removed by a validated retention cascade'], refuse: ['unremintable, mixed-version, or colliding claim material'] }),
