 ...
 * coverage key content-free (`cov-` plus 64 lowercase hex) at the connector AND at the ledger
 * CHECK: a storage identifier is not presentation material even once the alias-bearing
 * `github.core:${scopeAlias}:${rangeEnd}` form this canary was written for stopped being
 * storable. The projection names coverage rows by `(rangeStart, jobId)` instead, so a clean
 * projection has neither.
 */
