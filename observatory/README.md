# Pulseboard SDK v3 (public synthetic showcase only)

Source: `Chris0Jeky/Pulseboard` `observatory/adapters/build-sdk.mjs`, built for project `developer-lens`
from Pulseboard `main` at `0ed1dcb` (issue Chris0Jeky/Pulseboard#105). The artifact is
...
`<div data-pulseboard-bar style="min-height:2.5rem">` as the first child of `<body>`.
...
- `node observatory/check.mjs` verifies the lock hash, the `pulseboard-sdk 3.0.0` header, the collector
